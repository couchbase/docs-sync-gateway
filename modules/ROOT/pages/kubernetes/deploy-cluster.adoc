= Deploy SGW Cluster with CAO 1.x
...
:xref-pg-gettingstarted: xref:{version-sgw}@getting-started.adoc
:xref-pg-shared-bkt-access: xref:{version-sgw}@shared-bucket-access.adoc
:xref-create-an-rbac-user: {xref-pg-gettingstarted}#creating-an-rbac-user[this section, {window-new}]
...
[abstract]
This guide demonstrates how to deploy a Sync Gateway cluster on Kubernetes using CAO {version-cao-1-x}
...
.About Import Nodes
****
Prior to Couchbase Mobile 2.7 {xref-shared-bkt-access}, it was recommended that (only) one Sync Gateway node in a cluster be configured for handling document import processing.
...
Configuration of the Sync Gateway import node differs slightly from that of _non-import_ nodes (see xref:{version}@config-properties.adoc#databases-foo_db-import_docs[databases.$db.import_docs]).
...
.*Sync Gateway Enterprise Edition 2.7 and above*
NOTE: Use the _Import Node_ configuration for all nodes in your cluster.
...
. You will use a {url-k8-cfg-secret}/[Kubernetes Secret, {window-new}] to pass the configuration file to Sync Gateway on launch.
Alternatively, you could use a {url-k8-cfg-pod-cont}[Kubernetes configMap, window=_blank] if you are not concerned about security.
...
You will be using a {url-k8-wkld-ctrl-deploy}[Kubernetes deployment controller, {window-new}].
...
You can learn more about CPU resource assignment {url-k8-cpu-rsr-ass}[here, {window-new}].
...
Learn more about Kubernetes volumes {url-k8-storage-vols}/[here, {window-new}].
...
In the sample output below, we have Couchbase Server and Sync Gateway pods running in the same {url-k8-nmspc-wlk-thru}[namespace, {window-new}].
