...
----
<.> The `import_docs` property is used to specify that a {sgw} node participates (exclusively) in {xref-sgw-pg-import-process}. The mechanism by which {sgw} incorporates changes to data buckets it shares with {cbs} -- see: <<import-process>>.
