...
WARNING: {sgw} maintains the sync metadata internally, and its structure can change at any time. Applications must not use it for business logic. We do not support directly using N1QL queries or modifying the internal sync metadata contents in production environments. +
