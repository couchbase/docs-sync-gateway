...
On Sync Gateway, the pruning algorithm retains a configurable number of revisions (revs_limit) and removes all older revisions.
...
Tombstones can also be manually removed via Sync Gateway's xref:rest-api:rest_api_admin.adoc#tag/Document/operation/post_keyspace-_purge[+/{keyspace}/_purge+] endpoint.
