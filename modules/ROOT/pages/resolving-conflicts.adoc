...
:param-abstract: This guide relates to handling conflicts created by Couchbase Lite 1.x clients.
include::partial$block-abstract.adoc[]

== Introduction
This content addresses conflicts created by Couchbase Lite 1.x clients.
It describes how to handle the conflict resolution on the server-side using the Sync Gateway Admin REST API. That information does not apply to applications using Couchbase Mobile 2.x _only_ -- see {xref-sgw-pg-resolving-conflicts}.
