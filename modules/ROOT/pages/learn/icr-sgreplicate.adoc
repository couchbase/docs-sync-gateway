...
:description: Use Couchbase Sync Gateway's Inter-Sync Gateway Replication to keep clusters in different data centers in sync.

include::partial$_std-hdr-sgw.adoc[]

:topic-group: {tg-rep-icr}
:param-related: {xref-sgw-pg-config-properties} | {xref-sgw-pg-rest-api-admin} | {xref-sgw-pg-icr-running}
:param-abstract: Inter-Sync Gateway Replication provides resilient, secure, scalable bidirectional synchronization of data between cloud and edge data centers. +
...
An ideal use-case for Inter-Sync{nbsp}Gateway Replication, which was designed to keep clusters in different data centres in sync.
...
*However*, Inter-Sync{nbsp}Gateway Replication is specifically designed for Sync Gateway deployment and it *must* be used for replication between clusters containing active Sync{nbsp}Gateway nodes.
...
=== Context
Sync Gateway supports the ability to run replications between Sync Gateway clusters using Inter-Sync Gateway Replication, with the {glos-term-active-replicator} {glos-term-synchronizing} changes between two {glos-term-sync-gateway-databases}.
...
Replications run using Inter-Sync{nbsp}Gateway Replication, are websocket-based.

NOTE: For users on releases prior to 2.8, SG Replicate provides a HTTP-based replication -- see the appropriate documentation here -- {xref-sgw-pg-icr-sgreplicate-sgr1} (deprecated).
...
=== Security
// tag::security[]
Transport level security is provided by HTTPS or WSS (for websockets{fn-sgr2}).
