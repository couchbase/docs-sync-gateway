= Inter-Sync Gateway Replication introduced in Sync Gateway 2.8
...
// Footnotes
:fn-sgr1: footnote:sgr1[inter-Sync Gateway replication pre-2.8]
:fn-sgr2: footnote:sgr2[inter-Sync Gateway replication in release 2.8+]
...
*However*, inter-Sync{nbsp}Gateway replication is specifically designed for Sync Gateway deployments and it *must* be used for replication between clusters containing active Sync{nbsp}Gateway nodes. 

Active Sync Gateway nodes are nodes with incoming writes; these may be from Couchbase Lite clients, the REST API, or through the Couchbase Server.
