...
The document expiration time is returned in the response of GET xref:rest-api.adoc#/document/get\__db___doc_[+/{tkn-db}/{doc}+] when `show_exp=true` is included in the querystring.
...
. Take the database offline using the xref:admin-rest-api.adoc#/database/post\__db___offline[+/{tkn-db}/_offline+] endpoint.
...
. Bring the database back online using the xref:admin-rest-api.adoc#/database/post\__db___online[+/{tkn-db}/_online+] endpoint.
