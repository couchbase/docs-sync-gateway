...
:description: Couchbase Mobile's SG Replicate protocol support inter-cluster replication

include::partial$_std-hdr-sgw.adoc[]

:topic-group: {tg-rep-icr}
:param-related: {xref-sgw-pg-config-properties} | {xref-sgw-pg-rest-api-admin}
:param-abstract: This content is deprecated. It provides an introduction to, and overview of SG Replicate, which was replaced by Inter-Sync{nbsp}Gateway Replication in release 2.8.
...
* Replication between Sync Gateway databases doesn't support automatic conflict resolution even when the no-conflicts mode is enabled (i.e {xref-sgw-pfx-config-properties}#databases-this_db-allow_conflicts["allow_conflicts": false]). Apps will continue to rely on the 1.x REST APIs to asynchronously detect and resolve conflicts.
...
The list of returned stats and their meaning can be found on the API reference of the {xref-sgw-pfx-admin-rest-api}#/server/get\__active_tasks[`_active_tasks`] endpoint.
