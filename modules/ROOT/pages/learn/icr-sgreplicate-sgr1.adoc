...
* Replication between Sync Gateway databases doesn't support automatic conflict resolution even when the no-conflicts mode is enabled (i.e xref:config-properties.adoc#databases-foo_db-allow_conflicts["allow_conflicts": false]). Apps will continue to rely on the 1.x REST APIs to asynchronously detect and resolve conflicts.
