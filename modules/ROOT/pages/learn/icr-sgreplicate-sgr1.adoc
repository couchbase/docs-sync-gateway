= Inter-Sync Gateway Replication v1.0
:page-layout: article
...
:param-abstract: This content was deprecated in Sync Gateway v2.8. It provides an introduction to, and overview of, Couchbase Sync Gateway's Inter-Sync Gateway Replication v1 feature, which was replaced by v2 at that release
...
* A warning message is logged whenever a version 1.0 Inter-Sync Gateway Replication is initialized (either through config or REST end point). The message emphasizes that this feature is deprecated.
