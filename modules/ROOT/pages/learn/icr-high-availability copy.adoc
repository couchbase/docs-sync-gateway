...
This will allow us to deploy a truly homogenous cluster even in SG-replicate based environment where every Sync Gateway node in cluster has an identical configuration. This will enable future platform architectural enhancements around mobile manageability .
...
Sync Gateway will automatically elect an appropriate node to run them on.
...
* If any nodes fails whilst running either replication, Sync Gateway elects a non-failing node to continue that replication on.
