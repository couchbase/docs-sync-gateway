...
Starting in Sync Gateway and Couchbase Lite 2.5, Couchbase Lite has the ability to automatically purge documents to which access has been revoked (via the Sync Function).

Note that in xref:2.1@sync-gateway::sync-function-api.adoc#access-removal-behavior[previous versions], revoking access to a channel did not delete such documents.
