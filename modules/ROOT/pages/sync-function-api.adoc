...
If that happens, the next time Couchbase Lite pulls changes from the gateway, it will purge the document from the database and send a "lost access" notification via the xref:couchbase-lite::swift.adoc#replication-events[document replication listener].
