...
+
Additionally, this API can be used in conjunction with a 3rd party server for the authentication process (see xref:{authentication-users}#custom-authentication[Custom authentication]).
+
Lastly, Sync Gateway supports xref:{authentication-users}#openid-connect[OpenID Connect authentication].
