...
Admin REST API::
Create a new user by sending a PUT request to {rest-api-admin--pfx}#/user/put\__db___user__name_[/\{db}/\_user/\{name}] or by sending a POST request to {rest-api-admin--pfx}#/user/post\__db___user_[/\{db}/_user], where `db` is the configured name of the database and `name` is the user name.
+
