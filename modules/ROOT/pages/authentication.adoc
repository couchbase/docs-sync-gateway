...
link:authentication.html#basic-authentication[Basic Authentication]::
Provide a username and password to authenticate users.
link:authentication.html#custom-authentication[Custom Authentication]::
Use an App Server to handle the authentication yourself and create user sessions on the Sync Gateway Admin REST API.
link:authentication.html#openid-connect[OpenID Connect Authentication]::
...
Admin REST API::
The user credentials (**username**/**password**) are passed in the request body to the link:admin-rest-api.html#/user/post\__db___user_[POST /+{db}+/_user] endpoint.
+
[source,bash]
----
...
link:authentication.html#implicit-flow[Implicit Flow]::
...
link:authentication.html#authorization-code-flow[Authorization Code Flow]::
