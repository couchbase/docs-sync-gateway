...
* {configuration-schema-database--xref--enable-xattrs} to enable convergence for a given database.
* {configuration-schema-database--xref--import-docs} to give a particular Sync Gateway node the role of importing the documents.
* {configuration-schema-database--xref--import-filter} to select which document(s) to make aware to mobile clients.
