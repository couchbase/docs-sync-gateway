...
You can also run `sgcollect_info` from the Admin REST API using the xref:rest_api_admin.adoc#tag/Server/operation/post__sgcollect_info[_sgcollect_info] endpoint.
...
<.> Using a Basic Auth to authenticate to an existing user.

When you set `admin_interface_authentication` to `true` in your Sync Gateway configuration, include proper authentication headers in your requests.
...
NOTE: When `admin_interface_authentication` is set to `true` in your Sync Gateway configuration, specify `--sync-gateway-username` and `--sync-gateway-password`.
