...
You can can use `sgcollect_info` to collect and save information locally, and/or to collect and upload the information to Couchbase -- see: <<ex-collect>>.
...
The `--sync-gateway-password` option is no longer available.
Attempting to use this option causes an error.

When you specify the `--sync-gateway-user` option, `sgcollect_info` first tries to use the value of the `SG_PASSWORD` environment variable as the password.
If this is not set, `sgcollect_info` prompts you to enter the password.

You must use the `--sync-gateway-user` option to collect the full set of diagnostics.
