...
The `--sync-gateway-password` command line argument has been removed from `sgcollect_info` for security reasons.
When connecting to Sync Gateway 3.3 or later, the password is now prompted interactively when using `--sync-gateway-username`.
If you try to use `--sync-gateway-password`, you'll see an error with instructions.
