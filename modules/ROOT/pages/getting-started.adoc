...
:sg_download_link: http://packages.couchbase.com/releases/couchbase-sync-gateway/2.5.1/
:sg_package_name: couchbase-sync-gateway-community_2.5.1_x86_64
:sg_package_name_ee: couchbase-sync-gateway-enterprise_2.5.1_x86_64
:sg_accel_package_name: couchbase-sync-gateway-enterprise_2.5.1_x86_64
:tabs:
...
<5> *Enterprise Edition*: By default, the `delta_sync.enabled` property is disabled on Sync Gateway.
When enabled, Couchbase Lite clients will switch to using delta sync automatically.
Similarly, if delta sync is disabled on Sync Gateway, clients will switch to normal mode.
This feature is described in xref:config-properties.adoc#databases-foo_db-delta_sync[databases.$db.delta_sync.enabled].
--
