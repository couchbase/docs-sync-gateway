...
* You must first create a Sync Gateway *database*.
A Sync Gateway database is a namespace for documents and multiple databases can be defined in the configuration file for Sync Gateway.
Each Sync Gateway *database* connects to a Couchbase Server *bucket*.
This configuration below creates a database called *getting-started-db* and connects to the *getting-started-bucket* which was added in the <<creating-a-bucket>> section.
+
