= Admin REST API
:page-aliases: refer/rest-api-admin.adoc, refer/admin-rest-api.adoc, admin-rest-api.adoc
:page-layout: article
:page-status:
:page-edition:
:page-role: -toc
:page-content: reference
:page-service-desc: _attachments/sync-gateway-admin.yaml
:page-service-desc-type: application/vnd.oai.openapi+yaml
:description: Description of the Sync Gateway Admin Rest API

include::partial$_std-hdr-sgw.adoc[]

:param-abstract: Use the API explorer to find out more about Sync Gateway's endpoints by functionality.
:param-related: {rest-api--xref}  |  {rest-api-metrics--xref}  |  {rest-api-client-app--xref}
:topic-group: REST API
include::partial$block-abstract.adoc[]

== API Explorer
The API explorer below groups all the endpoints by functionality.
You can click on a label to expand the list of endpoints and also generate a curl request for each endpoint.

json_config_ui::attachment$sync-gateway-admin.yaml[]

swagger_ui::attachment$sync-gateway-admin.yaml[]

include::partial$block-related-content-api.adoc[]
