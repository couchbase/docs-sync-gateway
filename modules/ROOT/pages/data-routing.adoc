...
A typical example is a document that represents a shared resource (like a chat room or photo gallery). The document has a `members` property that lists the users who can access the resource.
...
** *Note 2:* The *star* channel doesn't mean that the user is granted access to all channels. It is only being granted access to 1 channel which contains **all documents**. This distinction is important when using the link:sync-function-api.html#requireaccesschannels[requireAccess()] Sync Function method.
