= Channels
:page-aliases: data-routing.adoc, learn/sync-gateway-channels.adoc
:idprefix:
:idseparator: -
...
:param-abstract: Sync Gateway's Channels are a key part of a flexible approach to data routing
...
A user can be given access to the *star* channel through the sync function or in the configuration file (see xref:{sgw-pg-read-access}#add-access[Add Access]).
...
You assign documents to channels by writing a Sync Function in the configuration file ({legacy-configuration-properties-pfx}#databases-this_db-sync[databases.$db.sync]).
...
xref:couchbase-lite:swift:learn/swift-replication.adoc#replication-events[Swift] | xref:couchbase-lite:java:learn/java-replication.adoc#replication-events[Java] |  xref:couchbase-lite:android:learn/java-android-replication.adoc#replication-events[Android (Java)] | xref:couchbase-lite:csharp:learn/csharp-replication.adoc#replication-events[C#] | xref:couchbase-lite:objc:learn/objc-replication.adoc#replication-events[Objective-C]
