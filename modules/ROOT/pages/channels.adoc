...
You assign a document to the _public_ (*!*) channel using the {sync-function--xref-channel-cmd} function.
...
You make dynamic user access grants in the sync function using the {sync-function--xref-access-cmd} method -- for more, see: {sync-func--xref-add-cmd}.
...
You can use the admin REST API to see the channels that documents are assigned to -- see: <<ex-alldocs>>.
...
You can provide this function using the {rest-admin-api-database--xref}.
Either embedding the Javascript in the providing configuration or include a reference to an external javascript function -- see: {configuration-javascript-functions--page}
// {legacy-configuration-properties-pfx}#databases-this_db-sync[databases.$db.sync]).
...
For example, based on the contents of the document, the Sync Function may call {channelfunc--xref} to add the document to one or more channels -- see <<ex-use-channel>>
...
If you don't supply a Sync Function in the configuration file, Sync Gateway uses the {configuration-schema-static--pfx}#databases-this_db-sync[default Sync Function].
