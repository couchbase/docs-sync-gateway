...
:param-related: {rest-api--xref}  |  {rest-api-admin--xref}  |  {xref-sgw-pg-stats-monitoring}  |  {xref-sgw-pg-stats-prometheus}
...
* The `metrics` endpoint returns Sync Gateway statistics in {glos-term-prometheus} format. +
...
The xref:rest_api_metric.adoc[API reference] groups all the endpoints by functionality.
