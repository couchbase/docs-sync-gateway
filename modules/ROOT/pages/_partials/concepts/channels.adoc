...
[#lbl-chan-metda-struct]
== Channel Metadata Structure

When troubleshooting channel assignments or access issues, it's helpful to understand how Sync Gateway stores channel information in the document's `_sync` metadata.

=== Channel Metadata Fields

Sync Gateway stores this metadata in the document's extended attributes (`XATTRs`) and includes three key fields that track channel membership over time.

==== `_sync.history.channels`

`_sync.history.channels` array shows the exact channels that each document revision belonged to, with one array per revision.
...
----
{
  "history": {
    "revs": [
      "2-e9615f134966a72a286fd270e15c4f",
      "1-38782707b39d3647026a86c3dabfb3"
    ],
    "channels": [
      [
        "Prm.Dst.1016527"
...
In this example, the channels array contains 2 entries corresponding to the 2 document revisions shown in the revs array.
Revision 2 (the current revision) belongs to channel `Prm.Dst.1016527`, while Revision 1 had no channel assignments and shows `"null"`.


==== `_sync.channels`

`_sync.channels` shows the complete history of every channel the document has belonged to.
It returns a `null` value for each document that still lives in a channel and a sequence number and revision `ID` for each document that has is not available in a channel.

[source,json]
----
{
  "channels": {
    "Prm.Dst.1016527": null
  }
}
----
...
`sync.channel_set` records the sequence number when the document first joined each channel.
When channel membership changes, it may append an end sequence number along with details about new channel assignments.
...
This indicates the document joined the `Prm.Dst.1016527` channel at sequence number `44575`.

You can access this metadata using the xref:rest-api/rest_api_admin.adoc#tag/Document[Admin REST API Document endpoint], which returns the document's `_sync` metadata in the response.
You can also access it using an {sqlpp} query:

[source,sql]
----
SELECT META().xattrs._sync FROM bucket WHERE META().id = "document_id"
----
