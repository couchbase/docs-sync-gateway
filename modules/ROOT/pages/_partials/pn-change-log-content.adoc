...
The top level `replications` configuration property and its sub-properties is deprecated. Instead, use the `database.replication` property (as above) to define Inter-Sync Gateway Replications, unless you specifically need to use v1 type Inter-Sync{nbsp}Gateway replications.
