...
+
Roles can be configured within using the DB section -- see: {configuration-properties--xref}. +
+
...
<.> Add the role the user is to be assigned to in `admin_roles` -- see: {configuration-properties--pfx}#databases-this_db-users-this_user-admin_roles[databases.$db.users.$user.admin_roles]
--
