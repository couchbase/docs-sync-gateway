...
* Inter-Sync{nbsp}Gateway Replication v1 and Inter-Sync{nbsp}Gateway Replication v2 replications can run on the same node, but you must ensure that they each have a different `replication_id`.
...
* Inter-Sync{nbsp}Gateway Replication v2 will continue trying to connect for 30 minutes following authentication failure (including user-invalid/doesn't exist).
...
.Replicating using Inter-Sync{nbsp}Gateway Replication v2
[CAUTION]
--
...
Both version 1 and version 2 can legitimately be in used at the same time, especially during transition.
