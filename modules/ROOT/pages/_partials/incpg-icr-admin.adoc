...
The configuration setting. `database.this_db.unsupported.sgr_tls_skip_verify`, can be used to skip the validation of TLS certificates, simplifying development and testing -- see: <<using-sgr-tls-skip-verify>> and the configuration item {configuration-schema-static--xref--databases-unsupp-sgr-tls-skip-verify}.
