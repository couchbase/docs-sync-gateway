...
+
--
For _Pull_ replications the _active_ Sync Gateway is responsible for detecting and resolving conflicts based on the configured _conflict_resolution_type_  -- see configuration item: {configuration-properties-xref--db-rep-conflict}.
...
You can learn more about the ($db.custom_conflict_resolver) property in the Configuration Schema Reference -- see: {configuration-properties-xref--db-rep-resolver}.
