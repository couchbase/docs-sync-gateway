...
|**Body**|**BulkGetBody** +
__optional__|List of documents being requested. Each array element is an object that must contain an id property giving the document ID. It may contain a rev property if a specific revision is desired. It may contain an atts_since property (as in a single-document GET) to limit which attachments are sent.|< <<_db_bulk_get_post_bulkgetbody,BulkGetBody>> > array|
|===
...
**Default** : `"configured `LogFilePath` location (e.g. `/home/sync_gateway/logs`)"`|string
|**redact_level** +
