...
__required__|Database name|string
|**Body**|**body** +
__optional__|Request body|object
|===
...
|===
|HTTP Code|Description|Schema
|**201**|Documents have been created or updated. The response object is an array with the status for each document submitted in the original request.|< object > array
...
|**Body**|**BulkGetBody** +
__optional__|List of documents being requested. Each array element is an object that must contain an id property giving the document ID. It may contain a rev property if a specific revision is desired. It may contain an atts_since property (as in a single-document GET) to limit which attachments are sent.|object|
|===
...
|===
|HTTP Code|Description|Schema
|**200**|The request was successful|< string, object > map
|===
