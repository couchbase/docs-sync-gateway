...
:keywords: openapi, rest, Sync Gateway Metrics REST API
:specDir: 
:snippetDir: 
:generator-template: v1 2019-12-20 (+ SimonD + HakimC changes)
:info-url: https://openapi-generator.tech
:app-name: Sync Gateway Metrics REST API

[abstract]
.Abstract
# Introduction  Sync Gateway manages access and synchronization between Couchbase Lite and Couchbase Server. The Sync Gateway Metrics REST API returns Sync Gateway metrics, in JSON or Prometheus-compatible formats, for performance monitoring and diagnostic purposes.
...
* xref:#+get__expvar+[Get all Sync Gateway statistics in JSON format]
...
| 200
a| Successfully returned statistics. For details, see [JSON Metrics](stats-monitoring-json.html).
...
* xref:#+get_metrics+[Get debugging and monitoring runtime stats in Prometheus Exposition format]
* xref:#+get_metrics+[Get debugging and monitoring runtime stats in Prometheus Exposition format]
...
| 200
a| Successfully returned statistics. For details, see [Prometheus Metrics](stats-monitoring-prometheus.html).
| 
...
a| *+per_db+* +
_optional_
a| This array contains stats for all databases declared in the config file &mdash; see [View Statistics and Metrics](stats-monitoring.html) for more details on the metrics collected and reported by Sync Gateway.
...
a| *+num_idle_query_ops+* +
_optional_
a| The total number of idle query operations.
