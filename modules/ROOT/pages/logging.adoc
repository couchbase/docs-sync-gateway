...
Log levels can be set in the configuration file (see the link:config-properties.html#2.1/logging-$level[logging.$level] reference).
...
All log keys and descriptions are described in the link:config-properties.html#2.1/logging-console-log_keys[logging.console.log_level] property reference.

=== Console Output Color

There is an option to color log output based on log level if link:config-properties.html#2.1/logging-console-color_enabled[logging.console.color_enabled] is set to ``true``.
...
Each log level and its parameters are described in the link:config-properties.html#2.1/logging-$level[logging.$level] property reference.
