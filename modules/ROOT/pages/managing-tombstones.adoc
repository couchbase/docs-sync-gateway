...
NOTE: Setting additional properties on tombstones may appear to be successful through Couchbase Lite SDKs or the Sync Gateway REST APIs but those properties will not be persisted to Couchbase Server.

2+a|*Additional system properties on a tombstone*
|Persisted on the document
|Not persisted.

NOTE: Setting additional system properties (i.e `_exp`) on tombstones may appear to be successful through Couchbase Lite SDKs or the Sync Gateway REST APIs but those properties will not be persisted to Couchbase Server (and therefore not synchronized to clients).
|===
