...
:topic-group: deploy
:param-related!:
// {xref-sgw-pg-config-properties}  |  {xref-sgw-pg-rest-api-admin} |    {xref-sgw-pg-config-properties-databases}  |  {xref-sgw-pg-config-properties-databases-sync}

:param-abstract: This content provides approaches for integrating Sync Gateway with other servers to build services that react to document changes.
...
By adopting the approaches documented here you can build responsive services that react to changes in documents, adding value to the end to end process.

The use cases for these approaches to Sync Gateway integration with other servers comprise:

* Sending notifications when specific documents change, for example, by email or SMS
* Customized auditing or logging

These approaches include using the following change indicators, a comparison of which can be seen in <<tbl-scenarios>>:
...
.Scenarios by API
[#tbl-scenarios,cols="1,^1,^1",width="80%"]
|===
...
In this section::
<<lbl-endpoint>>  |  <<lbl-methods>>  | <<lbl-websockets>>
