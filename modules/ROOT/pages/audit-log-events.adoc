...
|`ID`
|The actual number passed into `enabled_events: []`.

|`Name` 
|A short descriptive name, this will be outputted as a field in the audit log.

|`Description`
|A longer name, this is will be outputred as a field in the audit log.

|`DefaultEnabled`
|If `true`, this is enabled without explicitly listing in `enabled_events: []`.

|`Filterable`
|If `true`, this will always be enabled if audit logging is turned on. 
Any of the events with `Filterable` set to `true` in the `enabled_events: []` object will be no-ops.
...
|`MandatoryFields`
|Fields required to be set in a given Audit Event.
If any fields are not set, a no-op will occur.

|`OptionalFields`
|Fields that are not required to be set in a given Audit Event.
