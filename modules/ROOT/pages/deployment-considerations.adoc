...
Then, the Sync Function's xref:sync-function-api.adoc#requirerolerolename[requireRole()] method can be used to allow certain operations only if the user has that role.
