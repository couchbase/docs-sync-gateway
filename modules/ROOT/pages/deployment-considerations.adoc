...
In general, {url-curl}[curl], a command-line HTTP client, is your friend.
You might also want to try {url-httpie}[HTTPie], a human-friendly command-line HTTP client.
...
An additional useful tool is the admin-port URL */databasename/_dump/channels*, which returns an HTML table that lists all active channels and the documents assigned to them.
Similarly, */databasename/_dump/access* shows which documents are granting access to which users and channels.
...
If you're pretty sure you've found a bug, please https://github.com/couchbase/sync_gateway/issues[file a bug report] at our GitHub repository and we can follow-up accordingly.
...
Support portal: https://support.couchbase.com
...
Alternatively you can download standalone Curl for Windows: {url-curl}download.html.
