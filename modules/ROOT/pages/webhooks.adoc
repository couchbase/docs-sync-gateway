...
== Definition
You can define _Webhooks_ in Sync Gateway's {configuration-properties--xref} at the database level.

include::partial$common-cfg-ext-javascript.adoc[tag=intro]

Learn more about this property ($db.event_handlers) in the Configuration Schema Reference -- see: {configuration-properties--xref--eventhandlers}.
