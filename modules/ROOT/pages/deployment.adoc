= Deployment
:description: This article covers different aspects of using Sync Gateway and Couchbase Server during production.
...
The configuration of the Sync Gateway import node is slightly different than the "regular" or "non-import" Sync Gateway nodes (see {configuration-properties--pfx}#databases-this_db-import_docs[databases.$db.import_docs]).
Replicator node:: if you are using SG{nbsp}Replicate then there will be one designated replicator node whose configuration is different than the rest of the nodes -- see {legacy-sg-replicate--xref}.
...
In Sync Gateway 2.6, the {configuration-properties--pfx}#databases-this_db-cache[cache configuration] provides more options to tune Sync Gateway caching, enabling the Sync Gateway to vertically scale without potentially running into  Out-of-Memory issues.
...
- {configuration-properties--pfx}#databases-this_db-cache-channel_cache[Channel cache]: applies to cases where the number of channels can potentially grow unbounded.
- {configuration-properties--pfx}#databases-this_db-cache-rev_cache[Revision cache]: applies to cases with large document sizes.
...
In particular, every write operation gets processed by the {xref-sgw-pg-sync-function} and triggers notifications to other clients with read access, who then perform reads to get the new data.
