...
Up until Sync Gateway 2.6, the xref:config-properties.adoc#databases-foo_db-cache[cache configuration] has successfully supported deployments with a small to moderate number of channels (in the order of hundreds to tens of thousands of channels).

However, as deployments grow in size (in the order of millions of channels), those cache settings are not sufficient to manage memory allocation.
