...
=== Opt-out
To continue using the pre-3.x configuration mode ensure that you use the command-line interface option `disable_persistent_config=true`.
This ensures you can use the {configuration-properties--xref} and that *no* Admin REST API  changes are persisted across {sgw} restarts.
