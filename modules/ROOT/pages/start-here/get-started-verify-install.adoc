...
* Monitor the {sgw} revision number returned by the database endpoint (xref:rest_api_public.adoc#tag/Database-Management/operation/get_db-[GET /\{db}/]).
The revision number increments for every change that happens on the {sgw} database.
* Query a document by ID on the {sgw} REST API as shown in <<lbl-check-cbs>>. Use xref:rest_api_public.adoc#tag/Document/operation/get_keyspace-docid[GET /\{keyspace}/\{docid}] -- see: {rest-api-access--xref} for more.
