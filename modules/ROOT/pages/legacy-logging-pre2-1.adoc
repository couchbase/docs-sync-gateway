...
The descriptions and default values for each logging property can be found on the xref:{configuration-schema-static--page}[Sync Gateway configuration] page.
...
The *logFilePath* property can also be set in the configuration file at the {configuration-schema-static--pfx}#server-configuration[server level].
