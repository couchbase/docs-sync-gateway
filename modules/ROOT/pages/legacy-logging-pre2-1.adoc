...
The descriptions and default values for each logging property can be found on the xref:{legacy-configuration-properties--page}[Sync Gateway configuration] page.
...
The *logFilePath* property can also be set in the configuration file at the {legacy-configuration-properties-pfx}#server-configuration[server level].
