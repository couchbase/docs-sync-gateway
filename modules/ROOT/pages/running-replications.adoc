...
* Replication between Sync Gateway databases doesn't support automatic conflict resolution even when the no-conflicts mode is enabled . When running two Sync Gateway clusters with `allow_conflicts: false`, cross-cluster document conflicts will result. Deployments  must implement custom conflict resolver in an external app as specified [here](https://docs.couchbase.com/sync-gateway/2.1/resolving-conflicts.html).
