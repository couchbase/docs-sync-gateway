...
You can opt-in to auto-purge behavior using the replicator level option `purge_on_removal` in the REST API -- see {rest-api-admin-isgr--xref-purge-on-removal}.
...
At the point his option is turned on it only applies to revocations that occur after the point the replication is up to.
