...
== Caching
In this section::
<<lbl-ctrl>> | <<lbl-max-memory>> | <<lbl-size>> | <<lbl-sharding>> | <<lbl-deltasync>> | <<lbl-disable>>
...
You can specify the maximum amount of memory the revision cache should take up in MB, setting the value to `0` will disable any eviction based on memory at rev cache. 
...
This results in eviction behavior of the rev cache based on both the memory footprint and number of items in the cache.
