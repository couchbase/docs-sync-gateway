...
There are two ways to configure the size of the revision cache: through memory in MB using xref:configuration-schema-database.adoc#cache-rev_cache-max_memory_count_mb[`rev_cache.max_memory_count_mb`], and through size in number of document revisions in cache xref:configuration-schema-database.adoc#cache-rev_cache_size[`rev_cache.size`]. 
...
You can use the xref:configuration-schema-database.adoc#cache-rev_cache-max_memory_count_mb[`rev_cache.max_memory_count_mb`] setting to specify the maximum amount of memory the revision cache should take up in MB, setting the value to `0` will disable any eviction based on memory at rev cache. 

When set, this maximum memory limit will work in conjunction with the `cache.rev_cache.size` parameter, allowing you to configure limits to the number of documents and the maximum memory usage within the rev cache to reduce the risk of OOM issues.
