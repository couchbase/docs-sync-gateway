...
When set, this maximum memory limit will work with the `cache.rev_cache.size` parameter, allowing you to configure limits to the number of documents and the maximum memory usage within the rev cache to reduce the risk of OOM issues.
