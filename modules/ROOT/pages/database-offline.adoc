...
* Taking a database offline: xref:admin-rest-api.adoc#!/database/post_db_offline[`+POST /{db}/_offline+`]
* Taking a database online: xref:admin-rest-api.adoc#!/database/post_db_online[`+POST /{db}/_online+`]

By default, when Sync Gateway starts, it brings all databases that are defined in the configuration file online.
To keep a database offline when Sync Gateway starts, you can add the `offline` configuration property to the database configuration properties for the database, with the value `true` (see xref:config-properties.adoc#foo_db[database properties]).
...
// This xref doesn't exist xref:accelerator.adoc[Sync Gateway Accelerator]
