...
To keep a database offline when Sync Gateway starts, you can add the `offline` configuration property to the database configuration properties for the database, with the value `true` (see {configuration-properties-pfx}#databases-this_db[database properties]).
