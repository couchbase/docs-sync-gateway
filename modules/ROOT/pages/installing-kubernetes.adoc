...
The Sync Gateway nodes in a cluster are homogeneous in configuration with the exception of importer nodes.
...
The following sections cover the steps to deploy regular nodes or import nodes.
// or a single replicator node.

For example, to deploy a cluster with 4 regular nodes and 1 importer node you would run the steps below 2 times, each time with a different configuration and deployment controller file.
