...
. Create a new file called *sgw-config.json* with the following and replace the `server` key with the addressable Couchbase Server hostname.
+
[source,json]
----
{
...
. You will use a https://kubernetes.io/docs/tasks/configure-pod-container/configure-pod-configmap/[Kubernetes configMap] to pass the configuration file to Sync Gateway on launch.
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: sync-gateway
          image: couchbase/sync-gateway:2.2.1-enterprise <3>
