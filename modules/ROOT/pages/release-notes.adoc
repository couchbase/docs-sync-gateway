= Release Notes
:jira-url: https://issues.couchbase.com/browse
:url-issues-sync: https://github.com/couchbase/sync_gateway/issues
include::_attributesLocal.adoc[]

== 2.7.0

*Config Changes*

* Removed options (previously deprecated)
** maxCouchbaseConnections
** maxCouchbaseOverflow
** SGAccel related options:
*** skip_runmode_validation
*** runmode
*** channel_index
*** channel_index.writer
*** channel_index.num_shards
*** channel_index.seq_hashing
*** channel_index.seq_hashing.expiry
*** channel_index.seq_hashing.hash_frequency
*** channel_index.tombstone_compact_freq

** Bucket shadowing:
*** shadow
*** shadow.doc_id_regex
*** shadow.feed_type
*** deprecated.shadow
*** deprecated.shadow.doc_id_regex
*** deprecated.shadow.feed_type

*Additional options*
db.import_partitions - EE-only. Default: 16 - Valid range: 1-1024
// unsupported.use_stdlib_json - Doesn't need documenting. Left in for Couchbase support-use if required. EE-only, Default: false. Uses Go's stdlib JSON library, instead of a faster third-party JSON library .

*Changed*
db.import_docs - Now defaults to *true* in EE; remains false by default in CE.

*{ke}*

- https://issues.couchbase.com/browse/CBG-502[*CBG-502*] x.509 broken due to deprecated CertificateAuthenticator
- https://issues.couchbase.com/browse/CBG-463[*CBG-463*] Potential feedback loop when replicating large attachments
- https://issues.couchbase.com/browse/CBG-317[*CBG-317*] network errors while getting bulk get with multi channel with server ssl enabled


*{fixed}*

- https://issues.couchbase.com/browse/CBG-449[*CBG-449*] _removed entry not sent with delta sync
- https://issues.couchbase.com/browse/CBG-370[*CBG-370*] Sync gateway fails to start
- https://issues.couchbase.com/browse/CBG-364[*CBG-364*] Odd SG Installation errors
- https://issues.couchbase.com/browse/CBG-363[*CBG-363*] Return HTTP 413 when server rejects document due to size
- https://issues.couchbase.com/browse/CBG-362[*CBG-362*] Inconsistent handling between views and N1QL for tombstone removals
- https://issues.couchbase.com/browse/CBG-357[*CBG-357*] GoCB Bucket Incr amt=0 logic not taking 'def' into account
- https://issues.couchbase.com/browse/CBG-342[*CBG-342*] GoCB Bucket Incr amt=0 logic not handling errors correctly
- https://issues.couchbase.com/browse/CBG-324[*CBG-324*] Blip messages double-tagging user for redaction
- https://issues.couchbase.com/browse/CBG-321[*CBG-321*] Improve sg-replicate logging integration
- https://issues.couchbase.com/browse/CBG-309[*CBG-309*] Don't log expected REST errors to sg_error.log
- https://issues.couchbase.com/browse/CBG-306[*CBG-306*] Avoid panics as an error handling mechanism
- https://issues.couchbase.com/browse/CBG-288[*CBG-288*] Investigate TestN1qlQuery failures
- https://issues.couchbase.com/browse/CBG-286[*CBG-286*] SUBDOC operation attempted when server doesn't support them
- https://issues.couchbase.com/browse/CBG-284[*CBG-284*] Retry loops for bucket update ops have no backoff and no max retry count
- https://issues.couchbase.com/browse/CBG-280[*CBG-280*] Syncgateway doesn't import all documents while importing 50M docs ( build 2.1.3-4)
- https://issues.couchbase.com/browse/CBG-279[*CBG-279*] go-blip leaks nextFrameToSend goroutine under aborted multi-frame send
- https://issues.couchbase.com/browse/CBG-265[*CBG-265*] Post Upgrade fails to remove 2.0 era design docs
- https://issues.couchbase.com/browse/CBG-264[*CBG-264*] Post Upgrade "fails" in a cluster with no query nodes
- https://issues.couchbase.com/browse/CBG-253[*CBG-253*] Unused bulk_docs reserved sequences may not be released
- https://issues.couchbase.com/browse/CBG-225[*CBG-225*] Unresponsive REST API on startup
- https://issues.couchbase.com/browse/CBG-221[*CBG-221*] Admin/User views cannot handle both xattr and non-xattr simultaneously
- https://issues.couchbase.com/browse/CBG-189[*CBG-189*] Compact() shouldn't trigger import errors
- https://issues.couchbase.com/browse/CBG-159[*CBG-159*] CouchbaseServerVersion should use clusterCompatibility version

== 2.6.1

*{fixed}*

- {jira-url}/CBG-530[*CBG-530*] New attachment metadata not preserved on CAS retry during document update
- {jira-url}/CBG-514[*CBG-514*] running sgcollect_info on command with root privileges is overriding bin/sync_gateway file on MacOS
- {jira-url}/CBG-502[*CBG-502*] x.509 broken due to deprecated CertificateAuthenticator

== 2.6.0

*Deprecated Configs*

* The following configuration properties are being deprecated in this release and will be removed in a future release:
** `adminUI`
** `configServer`
** `databases.$db.feed_type`
** `pretty`
** `profileInterface`
** `log` (superseded by `logging`)
** `logFilePath` (superseded by `logging.log_file_path`)
* The `walrus` value on the `databases.$db.bucket` property is being deprecated. Sync Gateway instances using `walrus` for development should connect to a running instance of Couchbase Server.

*{nftr}*

* Cache {enh}
+
xref:index.adoc[Read more]

*{enh}*

- {jira-url}/CBG-260[*CBG-260*] Add configurable minimum TLS version
- {jira-url}/CBG-220[*CBG-220*] Limit total size of rotated logs

*{fixed}*

- {jira-url}/CBG-449[*CBG-449*] _removed entry not sent with delta sync
- {jira-url}/CBG-370[*CBG-370*] Sync gateway fails to start
- {jira-url}/CBG-364[*CBG-364*] Odd SG Installation errors
- {jira-url}/CBG-363[*CBG-363*] Return HTTP 413 when server rejects document due to size
- {jira-url}/CBG-362[*CBG-362*] Inconsistent handling between views and N1QL for tombstone removals
- {jira-url}/CBG-357[*CBG-357*] GoCB Bucket Incr amt=0 logic not taking 'def' into account
- {jira-url}/CBG-342[*CBG-342*] GoCB Bucket Incr amt=0 logic not handling errors correctly
- {jira-url}/CBG-324[*CBG-324*] Blip messages double-tagging user for redaction
- {jira-url}/CBG-321[*CBG-321*] Improve sg-replicate logging integration
- {jira-url}/CBG-309[*CBG-309*] Don't log expected REST errors to sg_error.log
- {jira-url}/CBG-306[*CBG-306*] Avoid panics as an error handling mechanism
- {jira-url}/CBG-288[*CBG-288*] Investigate TestN1qlQuery failures
- {jira-url}/CBG-286[*CBG-286*] SUBDOC operation attempted when server doesn't support them
- {jira-url}/CBG-284[*CBG-284*] Retry loops for bucket update ops have no backoff and no max retry count
- {jira-url}/CBG-280[*CBG-280*] Syncgateway doesn't import all documents while importing 50M docs ( build 2.1.3-4)
- {jira-url}/CBG-279[*CBG-279*] go-blip leaks nextFrameToSend goroutine under aborted multi-frame send
- {jira-url}/CBG-265[*CBG-265*] Post Upgrade fails to remove 2.0 era design docs
- {jira-url}/CBG-264[*CBG-264*] Post Upgrade "fails" in a cluster with no query nodes
- {jira-url}/CBG-253[*CBG-253*] Unused bulk_docs reserved sequences may not be released
- {jira-url}/CBG-225[*CBG-225*] Unresponsive REST API on startup
- {jira-url}/CBG-221[*CBG-221*] Admin/User views cannot handle both xattr and non-xattr simultaneously
- {jira-url}/CBG-189[*CBG-189*] Compact() shouldn't trigger import errors
- {jira-url}/CBG-159[*CBG-159*] CouchbaseServerVersion should use clusterCompatibility version

*{ke}*

- {jira-url}/CBG-463[*CBG-463*] Potential feedback loop when replicating large attachments
- {jira-url}/CBG-317[*CBG-317*] network errors while getting bulk get with multi channel with server ssl enabled
- {jira-url}/CBG-502[*CBG-502*] x.509 certificate authentication broken due to deprecated CertificateAuthenticator
