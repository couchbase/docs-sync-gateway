...
- {url-issues-sync}[*N/A*] Index Panic issue: the indexer throws an `index out of range` runtime error.
* *Workaround:* disable xref:2.1@index.adoc#guaranteed-availability[GSI indexing].
- {url-issues-sync}[*N/A*] Sync Gateway channel queries to the Couchbase Server may time out.
* *Workaround:* disable xref:2.1@index.adoc#guaranteed-availability[GSI indexing].
