...
Introduced in {sgw} 4.1, the cluster compatibility version lets you:
...
New features that do not depend on metadata shared by other {sgw} nodes activate immediately on each upgraded node regardless of the compatibility version.
These are considered node-local features, and include areas such as caching, read-only operations, general bug fixes, and performance improvements.
...
* All nodes are running {sgw} 4.1 or later.
...
The `cluster_compat_version` advances to `4.2` and new cluster-wide features introduced in 4.2 become active.
