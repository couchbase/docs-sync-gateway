...
The Inter-sync-gateway Replication (v1) global level `replications` configuration property is deprecated at his release.

Inter-sync-gateway Replication (v2) replications are now defined with a `replications` property at the individual database level; that is, as a member of the `database.this_db` properties.
