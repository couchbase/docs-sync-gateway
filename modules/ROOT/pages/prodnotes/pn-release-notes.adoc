...
:page-edition:
:page-role: Reference
:description: Couchbase Sync Gateway -- Product Release Notes
...
[abstract]
--
This content provides release information for Couchbase Sync Gateway {page-component-version}
--
...
However, root level replications (which by definition, use SG-Replicate 1.x) will continue to support remote replications, albeit in the now deprecated feature.
--
...
* Pre-Sync Gateway 2.8 -- only Inter-Sync Gateway Replication version 1 is available
* For Pre-Sync Gateway 2.8+ -- you may run Inter-Sync Gateway Replication version 2 and Inter-Sync Gateway Replication version 1 (deprecated)

SG versions compatible as a passive node as part of SGR1/SGR2 replications::

* Inter-Sync Gateway Replication v1 can target any Inter-Sync Gateway Replication version
* Inter-Sync Gateway Replication v2 can only target Inter-Sync Gateway Replication v2+
====
// end::upgrading[]
