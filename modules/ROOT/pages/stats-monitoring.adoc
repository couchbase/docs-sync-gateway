...
+
By default, that is the number of bytes received on `127.0.0.1` since node start-up.
Use Case::
...
+
By default, that is the number of bytes sent on `127.0.0.1` since node start-up.
...
+
By default, that is the number of bytes received on `0.0.0.0` since node start-up.
...
+
By default, that is the number of bytes sent on `0.0.0.0` since node start-up.
Use case::
To calculate throughput on the public interface --  `pub_net_bytes_recv` / `pub_net_bytes_sent`
...
Description::
Number of new replications starting per second
//(`/_changes?since=0`).
...
==== sync_function_time
Description::
The total time spent evaluating the _sync_function_ per push write.
Use case::
...
Description::
The number of delta replications running.
...
Where {query name} is a placeholder representing a _ _Valid Query Name_, as defined below:
...
Description::
Number of attachments transferred since node start-up.

=== sgr_num_attachment_bytes_transferred
Description::
Number of attachment bytes transferred since node start-up.

=== sgr_num_docs_failed_to_push
Description::
Number of documents that failed to be pushed since node start-up.

=== sgr_num_docs_pushed
Description::
Number of documents that were pushed since node start-up.
