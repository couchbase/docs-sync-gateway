...
The response is a JSON object with this schema, use the links to find out more about each item.

The statistics detailed in this schema are all reset at each node restart; that is they are not persisted. So all totals, counts and averages are the values accrured since the last node restart unless otherwise specified.
