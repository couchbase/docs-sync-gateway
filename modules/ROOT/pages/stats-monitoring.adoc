...
In addition to the `expvar` endpoint, you can try the *developer preview* of Sync Gateway's Metrics REST API, which exposes stats in a Prometheus compatible format -- see {xref-sgw-pg-rest-api-metrics}.

For more information on using _Prometheus_ and _Grafina_ to monitor Sync Gatewy -- see https://blog.couchbase.com/monitoring-sync-gateway-prometheus-grafana/
--
