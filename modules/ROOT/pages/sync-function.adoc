...
The Sync Function is a JavaScript function whose source code is stored in the Sync Gateway's database configuration file, on the xref:config-properties.adoc#databases-foo_db-sync[databases.$db.sync] property.
...
- xref:write-access.adoc#data-validation[Validate the document]
- xref:write-access.adoc#write-access[Authorize the change]
- xref:sync-gateway-channels.adoc#add-to-channel[Assign the document to channels]
- xref:read-access.adoc#add-access[Grant users access to channels]
