...
In addition to the {rest-api--xref}, Sync Gateway provides secure access to two REST API  targeted at specific user populations and functional areas, namely the:

* {rest-api-admin--xref}
* {rest-api-metrics--xref}

Each REST API is accessed through a different, <<lbl-port-cfg, user-specifiable>>, TCP port.
This makes it easy to control their physical exposure, perhaps keeping the Admin REST API secure behind your firewall.

{sgw-s} 3.0 brings additional layers of security to the Metrics and Admin interfaces by requiring Couchbase Server Role-Based Access Control (RBAC) authentication for all <<lbl-secure-users,secure API users>>, and by {sgw--xref}{security--page}[enforcing TLS encryption].
...
In order to submit Admin or Metrics REST API requests you must create Couchbase Server users for that purpose, and to then provide a valid set of Couchbase Server credentials for the pre-configured Couchbase Server user on each API request.
