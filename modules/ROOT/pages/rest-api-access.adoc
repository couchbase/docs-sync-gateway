...
{sgw-s} 3.0 BETA brings additional optional but default layers of security through {tls-encryption--xref} for all API traffic and Couchbase Server Role-Based Access Control (RBAC) authorization and authentication for all <<lbl-secure-users,Admin and Metrics API users>>.
...
TLS is enforced by default for all Couchbase Server connections in 3.0 BETA.
