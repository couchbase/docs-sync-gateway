...
The currently available roles will vary depending on the Couchbase Server release version:

* For those available with the latest Server release -- see: <<tbl-role-capabilities>>
* For earlier releases -- see <<tbl-ee-svr-sgw-roles>>. +
...
.{sgw-s} role availability by release (pre 7.0.2)
[#tbl-ee-svr-sgw-roles,cols="1,2,^1,^1,^1,^1",options="header"]
|===
