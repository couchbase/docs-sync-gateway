...
For more information, refer to the xref:sync-function.adoc[Sync Function] documentation.
...
This means that Sync Functions treats `requireAccess`, `requireUser` and `requireRole` calls as no-ops.
* During import, Sync Function executes with `oldDoc` set to `nil`.
...
|import_filter
|Used to decide if a document should be imported.
It checks the type property of the document.
If it is not 'mobile', the function returns false, otherwise, it returns true.
