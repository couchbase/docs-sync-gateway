= Glossary
:page-aliases: sgw-glossary, refer/refer-sgw-glossary.adoc, refer-sgw-glossary.adoc
:page-partial:
...
include::ROOT:partial$block-authors-notes.adoc[tag=wip]
...
* _Read More_: {sync-inter-syncgateway-overview--xref}
...
* _Read More_: {sync-inter-syncgateway-overview--xref}
* _Related Config Elements_: {configuration-schema-database--xref--db-rep-adhoc}  |  {configuration-schema-database--xref--db-rep-continuous}
...
Configuration property: {configuration-schema-database--xref--db-rep-resolver}
--
...
It is an optional feature {fn_post200} -- see: {configuration-schema-database--xref--allow-conflicts}.
...
Persistent replication is defined in the configuration file sync-gateway.json -- see {configuration-properties-legacy--xref}.
...
----
Mentioned in:
* xref:manage:managing-tombstones.adoc[Managing Tombstones]
* xref:ROOT:shared-bucket-access.adoc#metadata-purge-interval[Metadata Purge Interval]
