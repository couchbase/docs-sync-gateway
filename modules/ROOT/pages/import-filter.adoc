...
Import filters are defined at the collection level and are useful when working with large datasets. 
They allow you to retrieve only the necessary documents rather than the entire dataset. Import filters reduce the amount of data that needs to be processed, improving the performance of your queries and analysis. 
Sync Gateway imports all documents without a filter, so we recommend using an import filter unless there is a compelling use case against it.


== Function Provision

Use the {rest-api-admin--xref--database-configuration} Admin Rest API endpoint xref:clusters:modules/Admin REST API[/{keyspace}/_config/import_filter] to provision an import filter for a database using the `application/javascript` mime type.
