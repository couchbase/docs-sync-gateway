...
Starting with Sync Gateway 4.0+, the default conflict resolution strategy has changed from *Most Write Wins* (MWW) to *Last Write Wins* (LWW). This fundamental change aligns with the shift from revision trees to version vectors and enables better consistency across distributed deployments.
...
* Sync Gateway 4.0+ replicating with Sync Gateway < 4.0+ instances
