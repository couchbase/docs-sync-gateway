...
* A new channel or role is granted to a user in the {xref-sgw-pg-sync-function}.
...
Existing replications implementing custom {xref-sgw-pg-resolving-conflicts-legacy} strategies (via the REST endpoint) will continue to function in the same manner as for SG{nbsp}Replicate replications after upgrade
