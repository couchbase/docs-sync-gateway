...
The use of 3.0 BETA's Persistent Configuration feature is strongly recommended and it is the default operational mode when starting {sgw}.
...
[CAUTION]
--
The migration to 3.0 BETA configuration is a ONE WAY process; to continue using legacy-mode configuration see: <<lbl-3-0-upgrade-opt-out>>
--

Just startup a {sgw} node using your existing configuration properties file and {sgw} will take the appropriate upgrade path as shown in <<tbl-enhancedcfg>>.
...
* {Sgw-t} must have write access to the directory containing the existing configuration (the one to be converted).
It will be used to backup the existing configuration, and to write the upgraded config.
* If your existing configuration has multiple databases, all of the *server* fields used to connect to _Couchbase Server_ must match. Whilst the connection credentials used may differ between databases,
{sgw-t} will only use the first set of credentials for the bootstrap configuration file.
...
.Upgrade Paths
[#tbl-enhancedcfg,cols="1a,1a", options="header"]
|===
^| Configuration exists in Server bucket -- a node with this _group ID_ has already started ^| No configuration exists -- this is first node with this _group ID_ to start

|{sgw-s} ignores the configuration file and uses the configuration associated with the _group ID_ provided (or the default group if no _group ID_ is provided).
...
Failing to do so may result in the configuration being overridden by one configured on  a different {sgw} node that has already started and registered its configuration.

[#lbl-3-0-config-tls]
=== Secure Server Connection

Secure TLS Connection to Couchbase Server is enabled out-of-the-box in 3.0 BETA.
...
=== Continue to use Legacy-mode Configuration File
...
To restart a sync from zero: After the upgrade reconfigure the inter-Sync Gateway replication {configuration-properties--xref--db-replications} to use a different {configuration-properties--xref--db-rep-id}
...
.. Add the Inter-Sync Gateway replication configuration, including Javascript function as {configuration-properties--xref--db-rep-resolver}
.. Upgrade each cluster to Hydrogen
* Restart Outcomes
** When the active cluster is restarted it will automatically switch to Inter-Sync Gateway Replication
** Replication will continue from the last checkpoint, unless over-ridden.
** {enterprise} The replication will distribute automatically
* Conflict resolution -- Conflicts will be handled by the function  specified as the as {configuration-properties--xref--db-rep-resolver}
* Upgrade Constraint -- The `allow_conflicts` flag is ignored for any inter-Sync Gateway replication replications configured.
====
--
=====
