...
You access roles through the Admin REST API much like users are accessed, through URLs of the form {rest-api-admin--pfx}#/role[/\{db}/_role/\{name}].
...
Admin REST API::
You can assign a role to a user by sending a PUT request to {rest-api-admin--pfx}#/user/put\__db___user__name_[/\{db}/_user/\{name}] where `db` is the configured name of the database and `name` is the user name.
+
