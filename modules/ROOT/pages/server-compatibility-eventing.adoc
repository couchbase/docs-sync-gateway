...
+
NOTE: The `allow_sync_documents` setting is true by default.
...
You can use Eventing and Sync Gateway connected to the same bucket. 
When Eventing operates on server buckets in *read only* mode -- see {server-eventing-terminologies--xref-bindings} for how to do this.
