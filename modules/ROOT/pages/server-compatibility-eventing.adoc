...
You can use Eventing and Sync Gateway connected to the same bucket, when Eventing operates on the bucket in *read only* mode.

You must write your Eventing function to be *idempotent*; to behave correctly when the same mutation is seen more than once.
