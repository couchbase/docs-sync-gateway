...
Shared Bucket Access enables document changes in a Couchbase Server bucket to be made through the Couchbase SDKs, directly through N1QL queries or by Sync Gateway and vice versa.
The following diagram shows how different clients (mobile, web, desktop) read and write data from Couchbase Server.
