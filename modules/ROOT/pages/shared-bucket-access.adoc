...
* xref:config-properties.adoc#1.5/databases-foo_db-enable_shared_bucket_access[$dbname.enable_shared_bucket_access] to enable convergence for a given database.
* xref:config-properties.adoc#1.5/databases-foo_db-import_docs[$dbname.import_docs] to give a particular Sync Gateway node the role of importing the documents.
* xref:config-properties.adoc#1.5/databases-foo_db-import_filter[$dbname.import_filter] to select which document(s) to make aware to mobile clients.

When this feature is enabled, the REST API will include the following changes.

* Sync Gateway purging (xref:admin-rest-api.adoc\?v=1.5#/document/post\__db___purge[/+{db}+/_purge]) removes the document and its associated extended attributes.
* Sync Gateway document expiry (PUT xref:admin-rest-api.adoc\?v=1.5#/document/put\__db___doc_[+/{db}/{docid}+]) will tombstone the active revision.
...
To ensure tombstones are replicated to clients, you should set the server's metadata purge interval based on your expected replication frequency (see the xref:config-properties.adoc#1.5/databases-foo_db-enable_shared_bucket_access[$dbname.enable_shared_bucket_access] reference).
...
. Setup a xref:running-replications.adoc[push replication] from the Sync Gateway instance used for Bucket Shadowing to the Sync Gateway 1.5 instance.
