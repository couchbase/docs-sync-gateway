...
You can retrieve a document's expiration time, as it is returned in the response of GET xref:rest-api:rest_api_public.adoc#tag/Document/operation/get_keyspace-docid[+/{keyspace}/{docid}+] using `show_exp=true` as the querystring.
