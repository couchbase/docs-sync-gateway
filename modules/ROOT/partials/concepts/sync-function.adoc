...
//
// include::ROOT:partial$topic-group-access-control-concepts.adoc[]
...
image::ROOT:sync-function-context.png["Sync Function Context",300]
...
NOTE: The Sync Function uses the https://ecma-international.org/wp-content/uploads/ECMA-262_5.1_edition_june_2011.pdf[ES5 standard] of JavaScript syntax.
...
By default, every document in the collection is automatically assigned to a channel with the same name as the collection. This system automatically creates a channel with the collection's name. The assignment of all documents to the collection channel is functionally similar to assigning them to the xref:ROOT:access-control-concepts.adoc#lbl-alldocs-channel[All Documents Channel].
...
+
--
NOTE: Users running version v3.0+ must run with `disable_persistent_config=true`
