...
In _Pull_ replications the _active_ Sync Gateway detects and resolves conflicts using Last Write Wins strategy in Sync Gateway 4.x+.
For cross-version replication scenarios (4.x ↔ 3.x), the configured conflict resolver policy determines the winner or returns an error if it cannot resolve the conflict.
...
If conflicts occur, you should configure a `pushAndPull` replication when using Conflict Free mode.
...
See: {url-blog-doc-how-conflicts-work}
--
...
For detailed information about each conflict resolution policy and how they determine winning revisions, see xref:ROOT:glossary.adoc#conflict-resolver-policies[Conflict Resolver Policies].
...
This is useful when you want to apply greater selectivity to the automatic conflict resolution process.
For example, you want to apply a 'remote wins' policy only for a specific type of document - see the 'Use Policies' tab in <<simple-conflict-resolvers>>.
...
+
NOTE: When creating a new revision, do not provide a `_rev/_cv` property.
Sync Gateway generates a new revision ID/current version.
Use `delete mergedDoc._rev` and `delete mergedDoc._cv` to invalidate these properties.
...
include::ROOT:partial$common-cfg-ext-javascript.adoc[tag=intro]

You can provide your conflict resolver as either an inline or external JavaScript function.44
...
include::ROOT:partial$common-cfg-ext-javascript.adoc[tag=config-full]
...
Sync Gateway generates a new revision ID in the `_rev` property.
<.> Invalidate the `_cv` property.
Sync Gateway generates a new current version.
<.> Transcribe properties from source to target.
+

NOTE: The `IF` block is temporary.
It circumvents the known_issue https://issues.couchbase.com/browse/CBG-1335[CBG-1335], which results in a revision ID mismatch.
--
====
=====
