...
include::ROOT:partial$static_restapi/admin/index.adoc[tags=User_configurable_replication_properties]
...
Unless you inhibit this by adding an `"initial_state": "stopped"` parameter to the replication definition -- see: xref:configuration:configuration-schema-database.adoc#replications-replication_id-initial_state[initial_state].
