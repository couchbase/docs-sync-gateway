...
The algorithm keeps the configured number of revisions (`revs_limit`) in the shortest non-tombstoned branch and removes all older revisions from that branch.
For applications using Couchbase Lite 2.x+ with automatic conflict resolution, low `revs_limit` values, even as low as 1, provide optimal performance and storage efficiency.

[NOTE]
--
Do not set `revs_limit` below 100 when `allow_conflicts` = true
This setting may affect the conflict resolution process, as insufficient revision history may prevent resolving conflicts.
--

The default and minimum values of `revs_limit` are dependent on whether
xref:configuration:configuration-schema-database.html#database-allow_conflicts[allow conflicts]
...
In the animation below, the document has a conflicting branch.
When the shortest branch reaches the `revs_limit` threshold, pruning cuts it off.
The revision tree is not in a corrupted state and the logic that chooses the winning revision still applies.
This prevents certain merge operations (`n-way` merge) from resolving conflicts and reduces disk space that conflict resolution could free.


NOTE: Couchbase deprecated `allow_conflicts = true` in Sync Gateway 3.1.
Modern applications use `allow_conflicts = false` by default, which enables automatic conflict resolution and allows `revs_limit` values as low as 1.
...
NOTE: Setting `revs_limit` below 100 when `allow_conflicts` = `true` may affect conflict resolution due to insufficient revision history.
However, with automatic conflict resolution (`allow_conflicts` = `false`), applications can safely use much lower `revs_limit` values.

Default and minimum `revs_limit` values depend on conflict handling configuration.
Higher values primarily serve legacy conflict handling requirements:
...
Couchbase Lite 2.0 introduced automatic conflict resolution that dramatically reduces revision history requirements.
With `allow_conflicts` = `false`, the default configuration, Couchbase Lite resolves conflicts automatically during replication, eliminating the need for extensive revision trees.

During pull replication, Couchbase Lite detects conflicts and applies automatic resolution rules where the most recent change takes precedence and deletions have priority.
This conflict-free approach allows you to set `revs_limit` to low values—even 1—without affecting functionality.
...
This can cause the pruning algorithm to skip obsolete revisions or to prune revisions prematurely.
...
If both configs are set, this results in eviction behavior of the rev cache based on both the memory footprint and number of items in the cache. If any of these limits are reached, cache eviction will be performed.
...
Setting the value to `0` will disable any eviction based on memory at rev cache.
