= Logging
:page-aliases: ROOT:advance/logging.adoc, ROOT:logging.adoc
...
:fn-legacy-text: pass:q,a[For legacy Pre-3.0 see xref:configuration:configuration-properties-legacy.adoc[legacy Pre-3.0 configuration]]
...
|Query
|Query is used for Sync Gateway code related to {sqlpp} queries
...
Sync Gateway produces six separate log files, split by log level.
...
.Continuous Logging - Log File Outputs
...
|TRACE
|Extremely verbose diagnostic output, used for deep troubleshooting
|`sg_trace.log`
|`false`
|2 Days
|1 Day
...
|STATS
|Periodic snapshot of Sync Gateway's internal runtime statistics
|`sg_stats.log`
|`true`
|6 Days
|3 Days
...
NOTE: The `stats` log records a snapshot of Sync Gateway's internal statistics at a fixed interval, rather than logging discrete events like the other levels. That interval is controlled separately, by the `unsupported.stats_log_frequency` property (default `1m`), and is independent of the `max_age` and `max_size` rotation settings shown above.
