...
* You need the *Sync Gateway Dev Ops* role. Both the CLI and the REST API use it to authenticate against the node's Admin API, so it applies whichever method you use.
* To run `sgcollect_info` from the command line, you also need shell access to the Sync Gateway node and the path to the binary.
* To trigger a collection from the REST API without shell access, you need network access to the node's Admin API interface (port `4985` by default).
* To upload the collected diagnostics directly to Couchbase Support, you need a customer name. A Zendesk support ticket number is optional.
...
This redacts the logs at the `partial` level and uploads the redacted zip file to `uploads.couchbase.com` under customer `Acme` and ticket `123`.
By default, Sync Gateway deletes the local zip file once the upload succeeds.
The `--keep-zip` flag in the command above keeps `/tmp/sgcollect_info.zip` on the node after the upload completes.
--
====
=====
...
|`keep_zip`
|`--keep-zip`
|Set to `true` to keep the local zip file on the node after a successful upload. By default, Sync Gateway deletes it once the upload completes.
...
Both the CLI and the REST API produce the same files in the output zip file.
...
|`sg_error.log`, `sg_warn.log`, `sg_info.log`, `sg_debug.log`, `sg_trace.log`, `sg_stats.log`
|The continuous logging files, one per level. Which levels contain data depends on the node's logging configuration.

|`sg_<level>-<timestamp>.log.gz`
|Rotated and compressed versions of the files above. Each rotated file is collected separately and keeps its own name.
...
|The on-disk configuration file the node was started with, under its own name, for example `sgw-config.json`. Passwords and keys are removed.

|`sync_gateway.log`
|OS-level system stats, and the server configuration, the runtime configuration, each database configuration, the server status and the cluster information.

|`expvars.json`
|Exported variables (expvars) from the running node, which show runtime stats.

|`sgcollect_info_options.log`
|The command line arguments, or the REST request body, used to start this collection.
|===
...
|`sync_gateway`
|The Sync Gateway binary. Collected when `sgcollect_info` can find it, and needed to read the profiles.
|===
...
`sgcollect_info` uses the xref:{sgw-pg-logging}#continuous-logging[continuous logging] feature to collect the six leveled log files: `sg_error.log`, `sg_warn.log`, `sg_info.log`, `sg_debug.log`, `sg_trace.log`, and `sg_stats.log`.

These new log files are rotated and compressed by Sync Gateway.
When you run `sgcollect_info`, rotated log files are preserved as separate compressed files rather than being concatenated into the main log file.

For example, if you have *sg_debug.log*, and *sg_debug-2018-04-23T16-57-13.218.log.gz* and then run `sgcollect_info` as normal, both *sg_debug.log* and *sg_debug-2018-04-23T16-57-13.218.log.gz* appear as separate files inside the zip output folder.
