...
You can specify a filter function dynamically using the {rest-api-admin--xref--import-filter} endpoint, or you can define one when you set up a database.
...
Use the {rest-api-admin--xref--database-configuration} Admin Rest API endpoint {rest-api-admin--xref--database-configuration--post-db-config} to provision an import filter for a database using the `application/javascript` mime type.
