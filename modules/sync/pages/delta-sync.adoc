= Delta Sync
:page-aliases: ROOT:delta-sync.adoc
...
You can enable delta-sync on a per-database basic in your Sync Gateway configuration file using the `delta_sync` properties in xref:rest-api:rest-api-admin.adoc[Database Configuration] -- as shown in <<sample-cfg>>:
