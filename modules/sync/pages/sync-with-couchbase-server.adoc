...
:page-aliases: ROOT:shared-bucket-access.adoc, ROOT:sync-with-server.adoc, ROOT:sync-with-couchbase-server.adoc
...
include::ROOT:partial$block-standard-footnotes.adoc[tags=sgw1x5;cbs5x0]
...
.Using a WebApp
Attachments can be accessed through {sgw-t}'s REST API using the xref:rest-api:rest_api_public.adoc#tag/Document-Attachment/operation/get_keyspace-docid-attach[+/{keyspace}/{docid}/{attach}+] endpoint.
...
image:ROOT:workload-isolation.png[]
