...
:page-aliases: ROOT:configuration-environment-variables.adoc
...
include::ROOT:partial$feature-catalog.adoc[tags=config-env-vars-def]
...
To use environment variables in your configuration file, insert the variable name -- prefixed by a `$` symbol -- wherever you would normally put the configuration value you're substituting for -- see <<ex-syntax>>.

You can define inline default values using the notation `${var:-default}`.
Sync Gateway uses the default value if the referenced environment variable is empty or undefined.
...
This means changes to the values of the environment variables are not detected by Sync Gateway while it's running.
Such changes only become effective after a restart of Sync Gateway.

Any references to undefined variables found in the configuration cause {sgw} to throw an error.
This prevents {sgw} from processing an invalid configuration.


== Variable Definition

How you define the environment variables in your runtime environment depends on the platform used to host Sync Gateway.

As with the configuration file, you might need to define strings containing meta-characters (for example, symbols such as `$\*/`).

Depending on the shell environment, you can use a number of options:

* Enclose the string in single-quotes, which removes the special meaning of every character between the quotes.
...
Note -- This works in double-quote enclosed strings or in strings with no enclosing quotes, but not in single-quotes since as stated above, it removes the special meaning of the enclosed characters including any escape characters.
...
----
<.> Use quotes to prevent interpolation of the literal string's `$` symbols, or use "pa\$\$word"
...
----
<.> Note we use single-quotes because of the password value's {double-dollar} symbols, or use "pa\`$`$word"
