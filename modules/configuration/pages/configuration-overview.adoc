...
Configuration changes made to a node through the API endpoints propagate to other {sgw} nodes belonging to the same cluster (or to a user-defined subset of nodes) -- <<lbl-config-grp>>
...
NOTE: no Admin REST API changes persist across {sgw} restarts.
...
Changes made from 1 node propagate automatically to other nodes in the group.
