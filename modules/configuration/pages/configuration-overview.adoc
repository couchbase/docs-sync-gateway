...
:page-aliases: ROOT:configuration-overview.adoc
...
* *Bootstrap Startup* +
Use a minimal configuration file to bootstrap a {sgw} node and attach it to its Couchbase Server cluster; these files and their settings are node-specific -- see the {configuration-schema-bootstrap--xref}
...
{sgw} propagates configuration changes made to a node through the API endpoints to other nodes belonging to the same cluster (or to a user-defined subset of nodes) -- <<lbl-config-grp>>

* *Persistent Updates* +
Any database changes made using the API endpoints persist and survive {sgw} node restarts (this does not apply when running in file-based configuration mode).
+
NOTE: Using {sgw} with Couchbase Server's {server-eventing--xref} feature requires persistent configuration.
Databases created via file-based configuration are not recorded in the registry and Eventing cannot detect them for compatibility checks -- see {server-compatibility-eventing--xref}.
...
Your existing legacy Pre-3.0 configuration files are optionally converted automatically on start-up to the new format for {ec}.
...
image::ROOT:persistent-config.png[,750]
...
All changes persist across {sgw} restarts.
{sgw} initiates auto restarts for any change that requires one.
...
NOTE: {sgw} does not persist Admin REST API changes across restarts.
...
Each node in the group continues to have node-specific config.
...
{sgw} automatically propagates changes made from one node to other nodes in the group.

All nodes in a cluster belong, by default, to a common shared group `default`.

If you move a {sgw} node to a new group, it inherits the configuration associated with that group.
