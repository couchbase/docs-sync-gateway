...
Your existing legacy Pre-3.0 configuration files (optionally) are automatically converted, on start-up, to the new format for {ec}.
...
Or turn off authentication of REST API users -- for test purposes *only*
...
{sgw} persists all changes across restarts.
{sgw} automatically initiates restarts for any change that requires one.
