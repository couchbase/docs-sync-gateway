...
:page-aliases: ROOT:configuration-properties.adoc, ROOT:refer/config-properties.adoc, ROOT:config-properties.adoc, ROOT:configuration-properties-legacy.adoc
...
You can change many configuration settings using the {rest-api-admin--xref} *but Sync Gateway does not persist these beyond a restart*.
...
* How {sgw} conducts replications
* What security to use
* What logging options to apply, and
* Any customization of import filtering and synchronization.

You achieve the majority of the configuration using standard JSON syntax -- see <<lbl-schema,the schema>> for more.
...
Those features make it ideal for incorporating inline JavaScript, which you can use to provision, for example, `sync` and `import_filter` functions.
--
