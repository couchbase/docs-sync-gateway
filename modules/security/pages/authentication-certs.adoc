...
:page-aliases: deployment-considerations,configuring-ssl, ROOT:security.adoc, ROOT:authentication-certs.adoc
...
This method is used in the getting started xref:ROOT:getting-started.adoc#start-sync-gateway[section].
...
Also refer to the xref:ROOT:getting-started.adoc#configure-couchbase-server[Getting Started: Configure Couchbase Server] to configure the RBAC user with appropriate privileges for access by a Sync Gateway instance.
