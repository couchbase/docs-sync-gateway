...
Last-Write-Wins uses timestamp comparison to resolve conflicts deterministically across all clusters.
...
Configure Sync Gateway nodes with shared bucket access and import processing enabled.

Add these settings to the database configuration:
[source,json]
----
{
  "databases": {
...
Bi-directional XDCR requires both settings.
Shared bucket access allows Sync Gateway to coexist with direct SDK writes, while import processing handles replicated documents.
