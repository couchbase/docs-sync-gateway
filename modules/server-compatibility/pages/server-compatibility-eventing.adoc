...
:page-aliases: ROOT:server-compatibility-eventing.adoc
...
This page provides details on how {server-eventing--xref} relates to data changes in the Couchbase Mobile ecosystem.
...
[IMPORTANT]
--
Sync Gateway must be running with {configuration-overview--xref} enabled to support compatibility with Eventing.

Databases created via file-based configuration (when running with `-disable_persistent_config`) are not recorded in the Couchbase Server registry.
Without registry records, Eventing cannot:
