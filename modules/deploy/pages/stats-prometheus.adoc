...
:page-aliases: ROOT:stats-prometheus.adoc
...
In this instance it defines Sync Gateway's `metricsInterface` as being accessible on `sync_gateway:4986/metrics`.
...
include::ROOT:partial$prometheus-config.yaml[]
...
include::ROOT:partial$prometheus-sync-gateway-rules.yaml[]
