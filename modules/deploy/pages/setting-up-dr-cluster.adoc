...
Both clusters remain operational, with no fail-over through load balancer switching.
You must configure both clusters with `shared_bucket_access=true` and `import_docs=true`.
...
. Configure load balancer to route traffic primarily to the Primary cluster.
...
. [*Optional step -- for optimization*] Connect Sync Gateway to the Disaster Recovery cluster just long enough to create indexes.
...
. [Optional step -- for optimization] Turn on Sync Gateway in the Disaster Recovery cluster just long enough to create indexes.
