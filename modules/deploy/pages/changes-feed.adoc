= Changes Feed
:page-aliases: ROOT:server-integration.adoc, ROOT:changes-feed.adoc
...
:param-related: {webhooks--xref}  |  {xref-sgw-pg-stats-prometheus}
// :param-related: {webhooks--xref}  |  {sgw--xref}{integrating-external-stores--page}[External Stores]  |  {xref-sgw-pg-stats-prometheus}
...
{sgw-te} provides the ability to extend the replication process and build responsive services that react to changes in documents, adding value to the end to end process.
...
include::ROOT:partial$server-integration-scenario-table.adoc[]
...
The changes feed API is a REST API endpoint (xref:rest-api:rest_api_public.adoc#tag/Database-Management/operation/get_keyspace-_changes[`+/{keyspace}/_changes+`]) that returns a sorted list of changes made to documents in the database.
...
The last method (`websocket`) is specific to {sgw}.
...
You make a request to the continuous changes API and both you and {sgw} will hold the connection open “forever.”
<<lbl-websockets>>::
